exports.handler = async (event) => {
  ...
  if (!device_id || level === undefined) {
    return { statusCode: 422, body: JSON.stringify({ error: 'device_id and level required' }) };
  }
  ...
  try {
    ...
    return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: true, device_id, level: payload.level }) };
  } catch (err) {
